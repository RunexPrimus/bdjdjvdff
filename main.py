...
# Payment settings
PROVIDER_TOKEN = os.getenv("PROVIDER_TOKEN", "")  # "" usually ok for XTR; if errors, put token
MIN_DONATE = int(os.getenv("MIN_DONATE", "1"))
MAX_DONATE = int(os.getenv("MAX_DONATE", "100000"))
...
async def save_payment_db(pool, user, invoice_payload, provider_payment_id, amount, currency):
    now = utc_now()
    async with pool.acquire() as conn:
        await conn.execute(
            "INSERT INTO payments(user_id, username, invoice_payload, provider_payment_id, amount, currency, created_at) "
            "VALUES($1,$2,$3,$4,$5,$6,$7)",
            user.id, user.username if user.username else None,
            invoice_payload, provider_payment_id, amount, currency, now
        )
...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Donat qilish uchun /donate buyrug‘idan foydalaning (tugmasiz).",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup(kb)
    )
...
async def ask_image_count(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # This is the default text handler for image prompts; it will be called
    # only when no donate flow is active for the user (see donate_text_handler ordering).
    if not await force_sub_required(update, context):
        return
    await add_user_db(context.application.bot_data["db_pool"], update.effective_user)
...
# ---------- Donate (tugmasiz) flow ----------
async def donate_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Start donate flow: ask user to enter amount (1-100000) as plain text."""
    if not await force_sub_required(update, context):
        return
    await add_user_db(context.application.bot_data["db_pool"], update.effective_user)

    context.user_data["awaiting_donate_amount"] = True
    await update.message.reply_text(f"💰 Iltimos, yubormoqchi bo‘lgan miqdorni kiriting ({MIN_DONATE}–{MAX_DONATE}):")

async def donate_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    This handler intercepts plain-text messages *only* if the user is in donate flow
    (context.user_data['awaiting_donate_amount'] == True). Otherwise it returns None
    so other text handlers (e.g., image prompt) will run.
    """
    if not context.user_data.get("awaiting_donate_amount"):
        # not in donate flow — ignore here so next handler can process (image prompt)
        return
    ...
    # try parse int
    try:
        amount = int(re.sub(r'\D', '', text))  # remove non-digits (extra safety)
    except Exception:
        await update.message.reply_text(f"❌ Noto‘g‘ri format. Iltimos, faqat son kiriting ({MIN_DONATE}–{MAX_DONATE}).")
        return

    if amount < MIN_DONATE or amount > MAX_DONATE:
        await update.message.reply_text(f"❌ Miqdor ruxsat etilgan diapazondan tashqarida. ({MIN_DONATE}–{MAX_DONATE})")
        return

    # Build invoice payload and send invoice (XTR Stars)
    prices = [LabeledPrice(label="Donate", amount=amount)]
    payload = f"donate:{user.id}:{int(time.time())}:{amount}"

    try:
        await context.bot.send_invoice(
            chat_id=update.effective_chat.id,
            title="💝 Donate — Botni qo‘llab-quvvatlash",
            description=f"{amount} Telegram Star(s) orqali donat",
            payload=payload,
            provider_token=PROVIDER_TOKEN,
            currency="XTR",
            prices=prices,
            start_parameter=f"donate-{user.id}-{int(time.time())}",
        )
        # clear flag; we will still rely on successful_payment to confirm
        context.user_data.pop("awaiting_donate_amount", None)
    except Exception as e:
        logger.exception(f"[DONATE] send_invoice failed: {e}")
        await update.message.reply_text("❌ To'lov yuborilmadi. Iltimos, keyinroq urinib ko'ring.")
        context.user_data.pop("awaiting_donate_amount", None)

# Pre-checkout handler
async def precheckout_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Answer pre-checkout queries (required by Telegram payments)."""
    pre = update.pre_checkout_query
    try:
        await pre.answer(ok=True)
    except Exception as e:
        logger.exception(f"[PRECHECKOUT] {e}")

# Successful payment handler
async def successful_payment_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle successful payments and store in DB, then thank user in Uzbek."""
    msg = update.message
    sp = msg.successful_payment  # telegram.SuccessfulPayment
    user = msg.from_user

    invoice_payload = sp.invoice_payload
    provider_payment_charge_id = sp.provider_payment_charge_id or sp.telegram_payment_charge_id or ""
    amount = sp.total_amount  # integer: smallest units (for XTR often equals number of stars)
    currency = sp.currency

    # save to DB
    try:
        await save_payment_db(context.application.bot_data["db_pool"], user, invoice_payload, provider_payment_charge_id, amount, currency)
    except Exception as e:
        logger.exception(f"[SAVE PAYMENT] {e}")

    # craft display name (prefer last name if available otherwise first_name)
    display_name = user.last_name if user.last_name else (user.first_name or f"{user.id}")

    # send thank-you message in requested format:
    # e.g. "✅ Rahmat, Karimov! Siz 1 ⭐ yubordingiz."
    try:
        await msg.reply_text(f"✅ Rahmat, {escape_md(display_name)}! Siz {amount} ⭐ yubordingiz.", parse_mode="Markdown")
    except Exception:
        # fallback without markdown escaping if something odd
        await msg.reply_text(f"✅ Rahmat, {display_name}! Siz {amount} ⭐ yubordingiz.")
    ...
    try:
        await context.bot.send_message(
            ...
            text=(
                ...
                f"Provider payment id: `{provider_payment_charge_id}`"
            ),
            parse_mode="Markdown"
        )
    except Exception as e:
        logger.debug(f"[ADMIN NOTIFY] {e}")
...
def main():
    ...
    # Donate (tugmasiz) handlers
    app.add_handler(CommandHandler("donate", donate_command))
    # donate_text_handler must be added BEFORE generic text handler so it can intercept when flag set
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, donate_text_handler), 1)

    # Payment handlers
    app.add_handler(PreCheckoutQueryHandler(precheckout_callback))
    app.add_handler(MessageHandler(filters.SUCCESSFUL_PAYMENT, successful_payment_handler))

    # Existing callbacks / flows
    ...
    # Generic text handler for image prompts (added after donate_text_handler)
