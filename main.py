# bot_postgres_get_logs_fixed.py
import logging
import aiohttp
import asyncio
import re
...
async def check_sub_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await check_subscription(user_id, context):
        ...
        await query.edit_message_text(
            "⛔ Hali ham obuna bo‘lmagansiz. Obuna bo‘lib, qayta tekshiring.",
            reply_markup=InlineKeyboardMarkup(kb)
        )
...
async def get_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["translated"] = prompt  # Translation placeholder
...
async def generate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        while True:
            ...
            try:
                async with session.get(urls[0]) as check:
                    if check.status == 200:
                        break
            except Exception:
                pass
