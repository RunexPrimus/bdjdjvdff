...
BOT_USERNAME = os.getenv("BOT_USERNAME", "DigenAi_Bot")
...
async def private_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global MAINTENANCE_MODE
    # Check if user is in a conversation state (like donate, admin)
    # If so, let the conversation handler deal with it
...
async def donate_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lang_code = get_user_language(context, user_id)
    msg = update.callback_query.message if update.callback_query else update.message
    cancel_btn = InlineKeyboardButton(t(lang_code, "btn_cancel"), callback_data="back_main")
    await msg.reply_text(t(lang_code, "enter_donate_amount"), reply_markup=InlineKeyboardMarkup([[cancel_btn]]))
...
async def donate_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.text and update.message.text.strip().lower() in ["/start", "/cancel", "cancel", "bekor qilish"]:
        lang_code = get_user_language(context, update.effective_user.id)
...
async def my_account_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [
        [InlineKeyboardButton(t(lang_code, "btn_withdraw"), callback_data="withdraw"), InlineKeyboardButton(t(lang_code, "btn_api"), callback_data="api_access")],
...
async def api_access_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global MAINTENANCE_MODE
    q = update.callback_query
    await q.answer()
    if MAINTENANCE_MODE:
    ...
    lang_code = get_user_language(context, q.from_user.id)
    try:
        await q.edit_message_text(t(lang_code, "api_soon"))
    except Exception:
        pass
...
def build_app():
    ...
    app.add_handler(CallbackQueryHandler(api_access_handler, pattern=r"api_access"))
